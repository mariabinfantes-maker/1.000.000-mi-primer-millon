import { obtenerSqlSinPooling } from "@/data/db/cliente";
 ...
 * en cualquier entorno (no modifica datos existentes).
 */
async function main() {
  const sql = obtenerSqlSinPooling();
  ...
  console.log("Tablas presentes:", (tablas as { table_name: string }[]).map((f) => f.table_name).join(", "));
