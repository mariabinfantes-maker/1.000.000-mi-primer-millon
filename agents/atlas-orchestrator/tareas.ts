 ...
 *
 * Esta tabla se escribió **leyendo el `process.argv` de cada módulo**,
 * no sus nombres. Al hacerlo aparecieron cinco casos en los que el nombre
 * engañaba, anotados abajo. Deducir del nombre es un error que ya se
 * cometió una vez con `convertir-verificacion`.
 */
 ...
 * sencillamente no se puede ejecutar, y esa es la garantía. Cuando entre un
 * proceso nuevo, se clasifica en el mismo cambio que lo crea, con sus
 * pruebas — la prueba «todo script de tsx está clasificado» lo reclama
 * sola.
 *
 ...
 * gasto. Por el nombre parecería lo contrario; se clasificó leyendo el
 * código.
 *
 * ── Los cinco casos en los que el nombre engañaba ─────────────────────
 *
 * 1. `repesca-verificacion` NO acepta ningún argumento: lee `descartes.json`
 *    directamente, aunque su nombre sugiera que se le pasa algo.
 * 2. `verificar-despliegue` EXIGE `--url`, y además acepta `--comparar-con`
 *    y `--probar-bloqueo`.
 * 3. `verificar-neon` acepta `--env`.
 * 4. `copia-seguridad-afiliacion` acepta `--env`.
 * 5. `migrar-a-neon` acepta `--env` y `--forzar`.
 */
